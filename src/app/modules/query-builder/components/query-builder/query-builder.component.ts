...
export class QueryBuilderComponent
  ...
  removeRuleSet(index: number): void {
    ...
    if (this.config.removeRuleSet) {
      ...
      parent.ruleSetMapping.splice(this.ruleSetIndex, 1);
    }
  ...
  getButtonGroupContext(): ButtonGroupContext {
    if (!this.buttonGroupContext) {
      this.buttonGroupContext = {
        ...
        ruleSetIndex: this.ruleSetIndex,
        $implicit: this.data,
      };
    }
  ...
  getFieldContext(rule: Rule): FieldContext {
    if (!this.fieldContextCache.has(rule)) {
      this.fieldContextCache.set(rule, {
        ...
        getDisabledState: this.getDisabledState,
        fields: this.fields,
        propertyFilterCtrl: this.propertyFilterCtrl,
        $implicit: rule
      });
    }
  ...
  getBoolConditionName(conditionName) {
    if (boolConditionName[conditionName]) {
      return boolConditionName[conditionName];
    }
